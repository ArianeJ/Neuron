...
int main(){
	...
	for(int i(1);i<nombreIntervalle;++i){
		...
		if((N1.GetTime<a) or (N1.GetTime>b)){                                     //Iext != 0 in [a,b]
			N1.prochaine_tension(0,h,1,Vreset,Vth);}
		else{
			N1.prochaine_tension(Iext,h,1,Vreset,Vth);}
		if((N2.GetTime<a) or (N2.GetTime>b)){                                     //Iext != 0 in [a,b]
			N2.prochaine_tension(0,h,1,Vreset,Vth);}
		else{
			N2.prochaine_tension(Iext,h,1,Vreset,Vth);}
		globalClock += h
		fichier<<globalClock<<" "<<N1.GetTension<<" "<<N2.GetTension<<endl;
